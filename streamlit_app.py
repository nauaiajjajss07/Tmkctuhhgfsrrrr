    ...
    try:
        while automation_state.running:
            ...
            try:
                ...
                log_message(f'{process_id}: Scrolling to load comments section...', automation_state)
                for _ in range(3):
                ...
                comment_box_selectors = [
                    ...
                    'div[aria-label*="write a public comment" i][contenteditable="true"]',
                    'div[aria-label*="comment" i][contenteditable="true"][role="textbox"]',
                ...
                comment_box = None
                for idx, selector in enumerate(comment_box_selectors):
                    try:
                        elements = driver.find_elements(By.CSS_SELECTOR, selector)
                            ...
                            try:
                                if element.is_displayed() and element.is_enabled():
                                    placeholder = driver.execute_script(
                                        "return arguments[0].getAttribute('aria-label') || arguments[0].getAttribute('data-placeholder') || arguments[0].getAttribute('placeholder') || '';", 
                                        element
                                    ).lower()
                                    
                                    # Try to click and check if it's editable
                                    driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", element)
                                    time.sleep(0.5)
                                    element.click()
                                    time.sleep(1)
                                    
                                    if 'comment' in placeholder or 'write' in placeholder or idx >= 4:
                                        comment_box = element
                                        log_message(f'{process_id}: ✅ Found comment box with selector #{idx+1}! Label: {placeholder[:50]}', automation_state)
                                        break
                            except:
                                continue
                        ...
                        log_message(f'{process_id}: Selector {idx+1} error: {str(e)[:50]}', automation_state)
                        continue
                ...
                submit_result = driver.execute_script("""
                    ...
                    const submitSelectors = [
                        ...
                        '[aria-label*="Post" i][role="button"]:not([aria-label*="like" i])',
                        'div[aria-label*="comment" i][role="button"]',
                        'button[type="submit"]',
                        'div[role="button"]:not([aria-label*="Close" i]):not([aria-label*="Cancel" i])'
                    ];
                    ...
                    for (let selector of submitSelectors) {
                        ...
                        for (let btn of buttons) {
                            if (btn.offsetParent !== null && btn.offsetWidth > 0) {
                                ...
                                // Check if it looks like a comment/post button
                                if (btnText === 'comment' || btnLabel.includes('comment') || 
                                    btnText === 'post' || btnLabel.includes('post comment')) {
                                    console.log('Clicking button:', btnLabel, btnText);
                                    btn.click();
                                    return 'button_clicked';
                                }
                            }
                ...
                if submit_result == 'button_not_found':
                    ...
                    log_message(f'{process_id}: Submit button clicked!', automation_state)
                
                # Wait and verify comment was posted
                time.sleep(3)
                
                # Check if comment box is cleared (indicates success)
                is_cleared = driver.execute_script("""
                ...
                if is_cleared:
                    ...
                    log_message(f'{process_id}: ⚠️ Comment may not have posted (box not cleared)', automation_state)
...
with tab3:
    ...
    st.info("💡 **Post UID Format:** `person_uid_post_uid` (e.g., `61561602301791_122176246694386743`)")
    
    post_uid = st.text_input("Post UID", value=st.session_state.comment_config.get('post_uid', ''),
                            placeholder="61561602301791_122176246694386743",
                            help="Format: person_uid_post_uid (get from post URL or permalink)")
    
    st.markdown("---")
    st.markdown("#### 🍪 Upload Multiple Cookie Files")
    st.caption("Each account will post one comment. Upload multiple cookie files to post multiple comments.")
    ...
    st.markdown("---")
    comment_delay = st.number_input("⏱️ Delay between comments (seconds)", min_value=1, max_value=300,
                                   value=st.session_state.comment_config['delay'],
                                   help="Wait time between each comment")
    
    st.markdown("#### 💬 Comments List")
    st.caption("Comments will be posted in rotation. One comment per account.")
    comments = st.text_area("Enter comments (one per line)",
                          value=st.session_state.comment_config['comments'],
                          placeholder="Amazing post!\nGreat content!\nLove this!",
                          height=150,
                          help="Enter each comment on a new line - they'll rotate across accounts")
    
    if st.button("💾 Save Comment Configuration", use_container_width=True):
        if post_uid and '_' in post_uid:
            # Generate the full post URL from UID
            generated_url = f"https://www.facebook.com/{post_uid.split('_')[0]}/posts/{post_uid}"
            st.session_state.comment_config['post_uid'] = post_uid
            st.session_state.comment_config['post_url'] = generated_url
            st.session_state.comment_config['delay'] = comment_delay
            st.session_state.comment_config['comments'] = comments
            st.success(f"✅ Comment configuration saved!\n\n📝 Generated URL: {generated_url}")
        else:
            st.error("❌ Invalid Post UID format! Use: person_uid_post_uid")
        st.rerun()
...
with tab4:
    ...
    if st.session_state.comment_config.get('post_uid'):
        st.info(f"📌 **Current Post UID:** `{st.session_state.comment_config['post_uid']}`")
    
    col1, col2 = st.columns(2)
    
    with col1:
        can_start = (not st.session_state.comment_automation_state.running and 
                    st.session_state.comment_config.get('post_uid') and 
        ...
        if not st.session_state.comment_config.get('post_uid'):
            st.warning("⚠️ Please configure Post UID first!")
